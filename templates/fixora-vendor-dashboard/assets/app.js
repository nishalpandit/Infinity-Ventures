/* Fixora Customer — shared Vanilla JS interactions */
(() => {
  'use strict';
  ...
  const nav = [
    ...
    {key:'notifications', icon:'bell', label:'Notifications', path:'notifications.html'},
    ...
    {key:'credit-group', icon:'rupee', label:'Bid Credits', path:'bid-credits/packages.html', children:[
      ...
      ['credit-history','Purchase History','bid-credits/purchase-history.html'],
      ['credit-transactions','Credit Transactions','bid-credits/transactions.html']
    ]},
  ...
  if(sidebarRoot){
    const links = nav.map(item => {
      ...
      const main=`<a class="nav-link ${active?'active':''}" href="${route(item.path)}" data-tooltip="${item.label}">${icon(item.icon)}<span class="nav-text">${item.label}</span></a>`;
      if(!item.children) return main;
    ...
    sidebarRoot.outerHTML = `<aside class="sidebar" id="sidebar" aria-label="Vendor navigation">
      <a class="brand" href="${route('dashboard.html')}"><span class="brand-mark">${icon('tool')}</span><span class="brand-copy"><strong>Fixora</strong><span>Vendor Portal</span></span></a>
  ...
  if(localStorage.getItem('fixora-vendor-sidebar') === 'collapsed' && innerWidth > 767) body.classList.add('sidebar-collapsed');
  $('#collapse-menu')?.addEventListener('click', () => { body.classList.toggle('sidebar-collapsed'); localStorage.setItem('fixora-vendor-sidebar', body.classList.contains('sidebar-collapsed')?'collapsed':'expanded'); });
  ...
  $('#profile-trigger')?.addEventListener('click', e => { e.stopPropagation(); $('#profile-dropdown')?.classList.toggle('open'); });
  ...
  $$('[data-confirm]').forEach(el=>el.addEventListener('click',e=>{
    e.preventDefault(); const type=el.dataset.confirm; const map={
      ...
      withdraw:['Withdraw this bid?','Withdrawing is permanent. The bid credit used for this application will not be returned.','Withdraw bid','Bid withdrawn']
  ...
  // Vendor bid submission consumes one permanent bid credit.
  $$('[data-submit-bid]').forEach(btn=>btn.addEventListener('click',()=>{
    const form=$('#bid-form');
    if(form){
      let valid=true;
      $$('[required]',form).forEach(f=>{const bad=!f.value.trim();f.classList.toggle('error',bad);valid=valid&&!bad;});
    ...
    localStorage.setItem('fixora-vendor-credits','4');
